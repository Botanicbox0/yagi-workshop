...
// Wave C v2 — Applicant distribution registration + metric log panel.
//
// Baked-in fixes:
...
//           brand fill (was muted outline, signal lost vs the call-to-action
//           moment).
...
export function DistributionPanel({
  ...
  // MED-6: empty state at action moment → CTA escalates to brand fill.
  ...
  return (
    <section className="rounded-card border border-edge-subtle bg-card p-6 md:p-8 space-y-6">
      ...
      {showForm ? (
        <form
          onSubmit={handleAdd}
          className="space-y-4 rounded-card border border-edge-subtle bg-muted/30 p-4"
        >
          <div className="space-y-2">
            <Label htmlFor="dist_channel">{t("channel_label")}</Label>
            <Select value={channel} onValueChange={(v) => setChannel(v as Channel)}>
              <SelectTrigger id="dist_channel" className="rounded-button">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {CHANNELS.map((c) => (
        ...
        <div>
          {/* MED-6: brand fill in empty-action state, outline otherwise */}
          <Button
            type="button"
...
function DistributionRow({ distribution }: { distribution: Distribution }) {
  ...
  const channelLabel = t(
    `channel.${distribution.channel}` as Parameters<typeof t>[0],
  );
  ...
  return (
    <li className="rounded-card border border-edge-subtle bg-background p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center rounded-pill px-2 py-0.5 text-[11px] font-medium bg-muted text-foreground border border-edge-subtle">
              {channelLabel}
            </span>
          </div>
          <a
      ...
      {editing ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div className="space-y-1">
              ...
              <Input
                ...
                className="rounded-button h-9 text-xs"
              />
            </div>
            <div className="space-y-1">
        ...
        <div className="flex items-center justify-between gap-3 text-xs">
          ...
          {/* MED-9: ghost button + Pencil icon (was bare anchor) */}
          <Button
            type="button"
            variant="ghost"
            size="sm"
