-- Phase 2.5 G8 hardening v2 — closes K05-003 remaining gap per Codex resume pass.
-- Extends validate_challenge_submission_content() to enforce the FULL
-- submission_requirements contract: required native_video/image/pdf/
-- youtube_url presence + object shape + image count bounds + YouTube URL
-- regex (mirrors src/lib/challenges/content-schema.ts + src/lib/validation/youtube.ts).
...
BEGIN
  ...
  -- Challenge state enforcement (unchanged from v1).
  ...
  -- Status enforcement (unchanged from v1).
  ...
  -- native_video: required-ness + shape
  ...
  -- image: required-ness + array type + count bounds
  ...
  IF jsonb_typeof(NEW.content->'images') = 'array' THEN
    ...
    END IF;
    -- each image must be an object with objectKey
    IF EXISTS (
  ...
  -- youtube_url: required-ness + regex validity (mirrors src/lib/validation/youtube.ts)
...
COMMENT ON FUNCTION public.validate_challenge_submission_content() IS
  'Phase 2.5 G8 hardening v2 (K05-003 full schema). Enforces entire submission_requirements contract on direct table writes: native_video/image/pdf/youtube_url required-ness + shape, image count bounds, YouTube URL regex, text_description bounds.';
