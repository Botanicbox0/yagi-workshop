...
10. Styling: design-system **v1.2 — Dark Brand UI** (3-tier, 60-30-10). **Dark is the main canvas and the default** (`:root` + `.dark` dark; `next-themes` `defaultTheme="dark"`; `.light` is opt-in secondary). Neutral tier is the 60% dominant: near-black bg (`#0A0A0A`), surface (`#161616`), light ink (`#F0F0F0`), muted ink (`#888888`), hairline borders (`#2A2A2A`) — **NEVER a pure-black `#000000` void or a light main canvas**. **Red `#ED1E1E`** = brand primary (~10%: CTAs, primary actions, active states; on-fill text `#FFFFFF`, on-dark text variant `#FF453A`); Tailwind family `brand` (`bg-brand`/`text-brand-on`/`accent-brand`/`bg-brand-soft`). **Gold `#FAD204`** = brand secondary (~5–15% support: small highlights, tags; on-fill text `#0A0A0A`). v1.1 Vermillion `#9A361F` + warm-ivory + sage `#71D083` all retired. Display = Editorial New (EN); body = Pretendard (KO) / Geist (EN). Pill CTAs, keep-all for Korean. Tailwind semantic state colors (`amber-500`/`red-500`/`green-500` etc.) are preserved — they are NOT brand colors (note: semantic `destructive` red and brand Red coexist by design). Runtime v1.1 hardcoded literals are swept; new UI must use v1.2 token utilities or standalone v1.2 email hex only. Source of truth: `.yagi-autobuild/design-system/` + PRODUCT-MASTER §AO + ARCHITECTURE §18.4.
