...
codex
I’ll do this as a read-only adversarial review. I’m going to inspect the target files and relevant line numbers first, then cross-check the Supabase PKCE API behavior against the code paths rather than assuming the implementation details.
exec
...
codex
I’ve read the core Supabase helpers and the new confirm route. Next I’m reading the locale pages, root middleware, and the legacy callback so I can evaluate redirect construction, coexistence, and middleware matching with exact surfaces.
exec
...
codex
The first pass shows the confirm route is self-contained and middleware excludes it as intended. I’m now checking package versions and Supabase SSR internals locally so the PKCE and cookie-chain conclusions are grounded in the installed code, not just comments.
exec
...
rg: package-lock.json: 지정된 파일을 찾을 수 없습니다. (os error 2)
rg: yarn.lock: 지정된 파일을 찾을 수 없습니다. (os error 2)
...
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content -LiteralPath '"'src/app/[locale]/auth/verify/page.tsx' | ForEach-Object { "'$i++; '"'{0,4}: {1}' -f "'$i, $_ }' in C:\Users\yout4\yagi-studio\yagi-workshop
 ...
 149:               <span className="ink-tertiary">쨌</span>
...
codex
Supabase’s public docs confirm the intended server-side `token_hash`/`type` pattern and that `RedirectTo` is the value passed by `signUp`/`resetPasswordForEmail`. I’m narrowing one area now: whether the installed SSR client writes the PKCE verifier into cookies in a way the route handler can read.
exec
...
rg: node_modules/@supabase/auth-js: 지정된 파일을 찾을 수 없습니다. (os error 2)
...
codex
One significant issue is emerging: in the installed `@supabase/auth-js` code, `verifyOtp({ token_hash, type })` posts only those parameters to `/verify`; it does not read or submit the PKCE code verifier. I’m checking the downstream reset page and route surfaces now so the recovery-specific verdict is precise.
exec
...
- Verdict: FAIL.
- Reasoning: `emailRedirectTo` is built as an absolute URL like `https://site/app/...`, and Supabase exposes that as `{{ .RedirectTo }}`. `sanitizeNext()` only accepts strings starting with `/`, so the template’s `next={{ .RedirectTo }}` will be rejected and replaced with `/onboarding/workspace`.
...
- Verdict: FAIL.
- Reasoning: A crawler GET with `token_hash` and `type` reaches `verifyOtp()` immediately. Because this route does not check the browser’s verifier cookie before verification, the crawler can still consume the single-use email token.
...
- Proposed fix: Align signup `sanitizeNext()` with `/auth/confirm`’s allowlist. Dashboard should include `SITE_URL`, plus final destinations such as `/onboarding/workspace`, `/onboarding/brand`, `/onboarding/invite`, `/app/**`, and `/reset-password` for each deployed origin.
