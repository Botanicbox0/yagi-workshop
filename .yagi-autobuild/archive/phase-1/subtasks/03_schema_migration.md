---
id: 03
name: Schema migration file (schema + RLS + storage policies)
status: pending
assigned_to: executor
---

# Subtask 03 — Schema Migration File

## Goal
Create the Phase 1.1 migration file. Write EVERYTHING into a single migration: schema (tables/triggers/helpers/indexes), RLS policies, storage buckets + storage policies. Do NOT run `supabase db push` — that happens in Subtask 04 behind a kill-switch.

## File to create

`supabase/migrations/20260421000001_phase1_schema.sql`

Create `supabase/` and `supabase/migrations/` dirs if they don't exist. Use Write tool.

## Full file contents

Paste this EXACT content into the file. Do not alter.
...
- [ ] Directory `supabase/migrations/` created
- [ ] File `supabase/migrations/20260421000001_phase1_schema.sql` exists with the exact content above
- [ ] Line count > 300 lines (sanity check)
- [ ] No `supabase db push` executed (that's subtask 04)
- [ ] No other files modified

## Notes for executor

- Use Write tool.
- Preserve exact content — including triggers, policies, and storage section.
