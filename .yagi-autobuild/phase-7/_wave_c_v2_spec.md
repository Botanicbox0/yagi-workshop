...
| **FU-D4** | **K-06 LOOP-1 FINDING 3 (LOW)** | **WorkPreview image figcaption uses raw filename (industrial). Pass `sub.title` as `workTitle` prop and render that as figcaption (or omit figcaption entirely on image branch — heading already states title).** | **editorial polish pass** |
| **FU-D5** | **K-06 LOOP-1 FINDING 4 + 6 (LOW)** | **(a) `<video controls>` portrait-orientation 9:16 viewport dominance — wrap in `max-h-[70vh] mx-auto` constrained container. (b) `/[locale]/app/my-submissions` no kind-guard — brand/artist users land on empty list with misleading "submit something" copy. Add kind-guard redirect or copy fork.** | **first portrait-format submission OR brand user complaint** |
| **FU-W6** | **K-06 LOOP-1 FINDING 5 (LOW)** | **`add_another_cta="다른 채널 추가"` reads off when `distributions.length === 0` (no first one yet). Add `add_first_cta="채널 추가하기"` and conditionally select. KO `또는` and `수치 입력` are fine as-is.** | **wording polish pass** |
