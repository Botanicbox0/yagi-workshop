...
### Q-090: Onboarding 카피 프레이밍 — "AI VFX" 좋아도, 사용자 선택 화면은 "AI 비주얼"

**Asked context:** Phase 2.8.1 SHIPPED 직후 야기가 onboarding role 선택 + client profile 폼 실제 화면을 보면서 UX 카피 재세팅 제안 (2026-04-27 새벽).
**Question:** Onboarding 수준의 제품 카피에서 "AI VFX" 쓰는 게 적절한가?
**Answer:**
1. 제품 정체성 수준 (landing, brand, marketing) 에서는 "AI VFX" / "AI Native Entertainment Studio" 유지 OK.
2. 사용자가 설정/선택하는 surface (onboarding role, profile setup) 에서는 "AI 비주얼 작업" 이 더 정확.
   - 이유: Workshop 의 실제 작업 종류는 VFX 외에도 이미지 / 캐릭터 / 디자인 / 3D 자산 / 디지털 휴먼 등을 포괄. "VFX" 단어 하나로 사용자 레인지 축소.
   - "AI 비주얼 작업" = 이미지 + 영상 + VFX + 3D 등 모든 시각 작업을 포괄.
3. "의뢰인" 카피 수정: "의뢰를 보내고 결과물을 받습니다" (수동) → "작업을 요청하고 결과물을 검토·관리합니다" (능동). Workshop = 협업 OS 라 클라이언트도 능동적 참여자.
4. Client profile 폼 재구조: contact name + email + phone 3개 필드를 `<fieldset>` 공통 그룹으로 묶어 "담당자 정보" legend 아래에 둘. mental model 명확해짐.
5. 동일 패턴을 향후 onboarding / settings 폼에서도 적용 — 관련 필드는 fieldset 으로 그룹화.

**Rationale:** 제품 정체성 워딩 (marketing copy) ≠ 사용자 선택 화면 워딩 (UX copy). 둘을 구분해서 marketing 은 강하게, UX 는 포괄적으로 쓴다. 카피 이동의 기준은 "더 많은 단어"가 아니라 "더 명확한 의미".

**Applies when:** 향후 onboarding / signup / profile setup / settings 폼 수정 시 카피 고르기. landing / hero / marketing 에는 적용 안 함.
**Confidence:** HIGH (야기 직접 확정 2026-04-27)
**Registered:** 2026-04-27 (Phase 2.8.1 SHIPPED post-fix)
